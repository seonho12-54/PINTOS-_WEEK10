...
- 한 기능이 여러 구현 지점으로 나뉘면 `### 5.N <기능 묶음>` 아래에 `#### 5.N.1`, `#### 5.N.2`처럼 함수/분기 단위로 쪼갠다.
- **`01-feature-overview-*.md`만 예외**: 전체 흐름 개요용이며 5장 생략 가능. 구체 구현은 `02` 이후 문서의 5장을 본다.
- **`02` 이후 feature 문서**: 5장에 해당 문서 주제의 구현 지점을 `### 5.N`으로 빠짐없이 둔다(공용 템플릿 5장 머리말 참고).
...
#### 5.1.1 `validate_user_buffer()` 버퍼 범위 검증
